/**
 * Svelte wrapper and mount helper for formular.dev
 */
import { writable } from 'svelte/store';
import { FAAdapter } from '@formular/atomos';

export function createSvelteForm(schema: any) {
  const fieldsData = Object.keys(schema.shape).map((key, i) => ({
    id: i,
    name: key,
  ...
  const fieldsStore = writable(fieldsData);
  const errorsStore = writable<Record<string, string>>({});
  const isSubmittingStore = writable(false);

  const adapter = new FAAdapter(
    fieldsData as any,
    {
      onFieldChange: (name, val) => {
        fieldsStore.update((prev) =>
      ...
      onErrorChange: (name, err) => {
        errorsStore.update((prev) => ({ ...prev, [name]: err }));
      }
    },
    schema
  );
  ...
  return {
    ...
    submit: async () => {
      ...
      try {
        fieldsStore.update((prev) => prev.map((f) => ({ ...f, touched: true })));
...
export function mountSvelteForm(
  ...
  const formStore = createSvelteForm(schema);
  ...
  container.innerHTML = `
    <form id="svelte-form" class="space-y-6">
  ...
  const htmlForm = container.querySelector('#svelte-form') as HTMLFormElement;
  ...
  ['firstName', 'lastName', 'email', 'password'].forEach((id) => {
    ...
    inputs[id].addEventListener('input', (e) => {
      formStore.handleChange(id, (e.target as HTMLInputElement).value);
    });
    inputs[id].addEventListener('blur', () => {
      formStore.handleBlur(id);
    });
  });

  // Keep local copies of store values to compute validation classes reactively
  ...
  const updateValidationStyles = () => {
    currentFields.forEach((f) => {
      ...
      const err = currentErrors[f.name];
  ...
  const unsubFields = formStore.fields.subscribe((fieldsData) => {
  ...
  const unsubErrors = formStore.errors.subscribe((errorsData) => {
    currentErrors = errorsData;
  ...
  const unsubSubmitting = formStore.isSubmitting.subscribe((isSubmitting) => {
  ...
  const handleSubmit = async (e: Event) => {
    e.preventDefault();
    try {
      const data = await formStore.submit();
      if (!data) {
